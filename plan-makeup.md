...
## Part E. 단일 상태 버튼 + 6hdl 다음날 입력 (승인됨)

### E-1. 오늘 할 일 행 — 우측 단일 상태 버튼 [완료]
- 좌측 체크 원 제거. 행 = [아이콘+과제명] … [상태 버튼]
- ①시작 전 [▶ 학습하러 가기](초록 채움) → ②학습 중 [완료 확인하기(자동인증)/완료했어요(수동)](아웃라인) + 옆 작은 ↗(사이트 재열기) → ③[인증 중…] → ④[✓ 완료] pill
- 미완 판정 시 ②로 복귀 + 안내 한 줄 (기존 notComplete)
- "못했어요"/6hdl 패널/만회일 패널/미완료 요약 배너 제거 (당일 사유 입력 폐지)

### E-2. 학생 첫 화면 — "못한 과제 돌아보기" 섹션 신설 [완료]
- 대상: 최근 7일 과거 날짜에 스케줄됐는데 체크 문서가 없는 과제 (과제 생성일 이후 날짜만)
- 항목별 [왜 못했지?] → 6hdl 사유 그리드 + 만회일 칩(기존 UI 이전) → not_done+reason+makeupDate 기록(원 날짜로)
- 입력하면 만회 과제 섹션으로 자연 이동. 만회 섹션 위에 배치
