...
import { buildAvatar, disposeAvatar, poseAvatar, DEFAULT_LOOK, BODY_BASE } from '/campus/lib/avatar.js';
...
export async function mountCampus(){
  ...
  //    ① 저작 데이터(ROOMS/PROPS) — 안정적인 id를 가진 단일 소스
  ...
  // ══ ① 저작 레벨 데이터 ═════════════════════════════════════════════
  // 복도 하나 + 룸 넷. 상단 2룸(문=남쪽) · 중앙 복도 · 하단 2룸(문=북쪽).
  // 전부 y=0 평면.
  //
  // ⚠ name 은 화면 표기 전용이다. id 는 저장·네트워크 경로에 쓰이므로 바꾸지 말 것.
  //   ('개인 자습실' 같은 이름은 나중에 바뀔 수 있다 — 그때 name 만 고치면 된다)
  const ROOMS = [
    {id:'class',  name:'교실',        sub:'클래스카드 · 오토보카 · 매일국어', x:-9, z:-17, w:16, d:10, door:'s', hue:0x1f7a33},
    {id:'office', name:'원장실',      sub:'입회 상담 · 레벨 테스트',         x: 9, z:-17, w:16, d:10, door:'s', hue:0x1f7a33},
    {id:'study',  name:'개인 자습실',  sub:'플래닝 · 자습 인증',              x:-9, z:  1, w:16, d:10, door:'n', hue:0x1f7a33,
     personal:true,
     go:'/campus/planning-room'},
    {id:'lounge', name:'휴게실',      sub:'쉬는 시간 · 잡담',                x: 9, z:  1, w:16, d:10, door:'n', hue:0x1f7a33},
  ];
  const HALL = {id:'hall', name:'복도'};
  const CORRIDOR = {minX:-17, maxX:17, minZ:-12, maxZ:-4};
  const DOOR_W = 3.6, WALL_T = 0.4, LOW_H = 1.35;
  ...
  // 교실: 화이트보드 + 책상 6
  prop('class-board', -9, -21.5, 8, 0.3, 1.7, 0xf7fbf7);
  for (let r = 0; r < 2; r++) for (let c = 0; c < 3; c++)
    prop(`class-desk-${r}${c}`, -14 + c*5, -18.4 + r*3.4, 3.4, 1.2, 0.75, 0xe8e1d4);
  // 원장실: 책상 + 소파 + 책장
  prop('office-desk', 9, -19.6, 5, 1.8, 0.78, 0xe4dbc9);
  prop('office-sofa', 9, -14.6, 4.4, 1.4, 0.7, 0x9fc9ac);
  prop('office-shelf', 16, -17, 0.9, 6, 2.2, 0xf0f4f1);
  // 개인 자습실은 고정 가구가 없다 — 로그인 계정의 배치를 불러와 그린다(아래 applyRoom)
  // 휴게실: 소파 2 + 테이블 + 자판기
  prop('lounge-sofa-a', 6, -1.6, 5, 1.3, 0.7, 0x8fc9a8);
  prop('lounge-sofa-b', 6,  3.4, 5, 1.3, 0.7, 0x8fc9a8);
  prop('lounge-table', 6, 0.9, 3.2, 1.6, 0.5, 0xf0f4f1);
  prop('lounge-vending', 15.4, -1.6, 1.2, 2, 2.1, 0x4fae7a);
  ...
  const lam = (c, map, lift = 0) => new THREE.MeshLambertMaterial({
    color: c, map: map || null,
    emissive: new THREE.Color(c).multiplyScalar(lift),
  });
  // flat = 조명을 아예 무시한다. 지정한 색 그대로 나온다.
  const flat = (c, map) => new THREE.MeshBasicMaterial({color:c, map:map || null});
  ...
  // ── 건물 바깥 ──────────────────────────────────────────────────
  // 줌아웃하거나 가장자리에 서면 맵 밖 빈 배경이 보였다. 지면을 넓게 깔아
  // 세상이 이어지게 한다. 먼 쪽은 안개가 배경색으로 녹여 경계가 안 보인다.
  {
    const g = new THREE.Mesh(new THREE.PlaneGeometry(400, 400).rotateX(-Math.PI/2), flat(0xe4ebe0));
    g.position.set(0, -0.06, -8);
    scene.add(g);

    // 건물을 둘러싼 산책로 — 실내 흰 바닥과 잔디 사이의 완충
    const path = new THREE.Mesh(new THREE.PlaneGeometry(46, 40).rotateX(-Math.PI/2), flat(0xf2f4ef));
    path.position.set(0, -0.04, -8);
    scene.add(path);

    // 나무 — 맵 경계 바깥에만 둔다(게임플레이 평면에 영향 없음, 충돌 없음)
    const trunk = lam(0xc4ab8c, null, 0.25), leaf = lam(0xadc4a8, null, 0.3);
    const TREES = [
      [-24,-26],[-15,-28],[-4,-29],[7,-28],[17,-27],[24,-22],
      [26,-12],[26,-2],[24, 9],[15, 12],[4, 13],[-7, 12],[-17, 11],[-25, 6],[-26,-6],[-25,-16],
    ];
    for (const [x, z] of TREES){
      const t = new THREE.Group(); t.position.set(x, 0, z);
      const tr = new THREE.Mesh(new THREE.CylinderGeometry(0.22, 0.3, 1.5, 8), trunk);
      tr.position.y = 0.75; tr.castShadow = true;
      const lv = new THREE.Mesh(new THREE.SphereGeometry(1.25, 14, 12), leaf);
      lv.position.y = 2.3; lv.scale.set(1, 0.92, 1); lv.castShadow = true;
      t.add(tr, lv); scene.add(t);
    }
  }

  floorPlate(-18, 18, -22.5, 6.5, 0xeceae7, 2, 0);          // 룸 사이 여백 — 한 단계 더 어둡게
  floorPlate(CORRIDOR.minX, CORRIDOR.maxX, CORRIDOR.minZ, CORRIDOR.maxZ, 0xffffff, 2, 0.01);  // 복도 = 흰 타일
  for (const r of ROOMS)
    floorPlate(r.x - r.w/2, r.x + r.w/2, r.z - r.d/2, r.z + r.d/2,
               0xf2f1ed, 2, 0.01);   // 룸 바닥 = warm white. 복도(순백)와 명도로만 갈린다

  // 바닥이 조명을 무시하는 대신 그림자도 못 받는다. 그림자만 그리는 투명 평면을
  // 한 겹 덮어 접지감을 살린다(이게 없으면 아바타가 공중에 뜬 것처럼 보인다).
  const shadowCatcher = new THREE.Mesh(
    new THREE.PlaneGeometry(40, 34).rotateX(-Math.PI/2),
    new THREE.ShadowMaterial({opacity: 0.15}));
  shadowCatcher.position.set(0, 0.02, -8);
  shadowCatcher.receiveShadow = true;
  scene.add(shadowCatcher);

  // 벽 — 시각 높이만 여기서 정한다. 충돌은 위에서 이미 만들었으므로 영향 없다.
  const wallMat = lam(0xf4f8f5, null, 0.42);   // 벽은 흰색. 카메라가 회전하므로 전부 허리 높이로 둔다
  for (const s of WALL_SEGS){
    const h = LOW_H;
    const m = new THREE.Mesh(new THREE.BoxGeometry(s.maxX - s.minX, h, s.maxZ - s.minZ), wallMat);
    m.position.set((s.minX+s.maxX)/2, h/2, (s.minZ+s.maxZ)/2);
    m.receiveShadow = true;
    scene.add(m);
  }
  // 룸 사이 빈 구간을 채우는 순수 장식 블록 (충돌 없음 — 이미 룸 벽으로 막혀 있다)
  for (const gx of [0]) for (const gz of [[-22,-12],[-4,6]]){
    const m = new THREE.Mesh(new THREE.BoxGeometry(2, LOW_H, gz[1]-gz[0]), wallMat);
  ...
  for (const p of PROPS){
    const m = new THREE.Mesh(new THREE.BoxGeometry(p.w, p.h, p.d), lam(p.c, null, 0.24));
  ...
  // 조명 — 전역 조명만. 천장 전등 같은 로컬 광원은 두지 않는다
  // 전체 노출: hemi + sun + 포인트 합이 1을 크게 넘으면 툰 셰이딩이 흰색으로 포화된다(민머리에서 특히 눈에 띔)
  // 합이 1.4를 크게 넘으면 툰 3단 램프가 최상단에 붙어 흰색으로 포화된다
  ...
  sun.shadow.radius = 2;
  scene.add(new THREE.AmbientLight(0xffffff, 0.11));   // 그림자 바닥을 살짝 띄운다
  ...
  let myLook = SAVED ? {...SAVED.look} : {...DEFAULT_LOOK};
  let myBody = SAVED ? {...SAVED.body} : {...BODY_BASE};

  // ── 개인 자습실 가구 ───────────────────────────────────────────────
  // 가구는 데이터다. 메시와 충돌 상자를 둘 다 이 데이터에서 만든다.
  const roomGroup = new THREE.Group(); scene.add(roomGroup);
  const ROOM_COLLIDERS = [];
  const FURN_MAT = {};
  const furnMat = t => (FURN_MAT[t] ||= lam(FURNITURE[t].c, null, 0.24));
  let myRoom = [];
  ...
  // 실시간 채널 — 개인 방은 계정별로 나눈다. 로그인 전엔 공용 채널로 둔다.
  const roomChannel = r => !r ? HALL.id
    : (r.personal && ME) ? `${r.id}:${ME.uid}`
    : r.id;

  // ── 플레이어 ──────────────────────────────────────────────────────
  let player = buildAvatar(myLook, myBody);
  scene.add(player.root);
  const meTag = nameTag(ME ? ME.name : '나');
  ...
  // ── 고정 NPC — 충쌤은 사람이 아니라 AI 페르소나라 항상 자리를 지킨다
  const NPCS = [
    {name:'충쌤', preset:TEACHER, x:14, z:-17.2, yaw:0},
  ].map(n => {
  ...
  const CAM_R = 21.8, CAM_H = 14.2;        // 부감 33°. 더 낮추면 허리벽이 시야를 가린다
  ...
  function resolve(p, axis){
    for (const c of COLLIDERS.concat(ROOM_COLLIDERS)){
  ...
  function setRoom(room){
    ...
    currentRoom = room;
    const label = !room ? HALL.name
      : (room.personal && ME) ? `${ME.name}님의 자습실`
      : room.name;
  ...
  function interact(){
    ...
    if (currentRoom.go) location.href = currentRoom.go;      // 룸 데이터가 이동 대상을 갖는다
  ...
  const cz = mountCustomizer({
    onChange(look, body){ myLook = look; myBody = body; rebuildPlayer(); },
  ...
  function frame(){
    ...
    // ── 입력 → 이동 벡터 (카메라 기준) ──
    // 꾸미는 동안에는 조작을 받지 않는다(모달 뒤에서 캐릭터가 걸어가면 안 된다)
    const frozen = cz.isOpen();
    let ix = 0, iy = 0;
    ...
    // 내 좌표 발행 — 룸이 바뀌면 net.js가 구독 대상을 통째로 갈아끼운다
    if (net) net.publish(P.x, P.z, P.yaw, moving, roomChannel(inRoom));
