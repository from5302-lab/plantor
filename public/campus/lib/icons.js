// ══════════════════════════════════════════════════════════════════
//  아이콘 — Lucide (ISC). 필요한 것만 인라인으로 들고 있다.
//
//  이모지를 아이콘 자리에 쓰면 플랫폼마다 그림이 다르고, 굵기·정렬이 UI 와 따로 놀아
//  즉시 '만들다 만' 인상을 준다(러그가 🟩 초록 네모로 보이던 게 그 예).
//  라인 아이콘 한 세트로 통일하면 굵기·크기·색이 텍스트와 같이 움직인다.
//
//  path 데이터는 node_modules/lucide-react 에서 뽑았다. 아이콘을 추가할 때도
//  거기서 가져와야 굵기(stroke-width 2)와 캡 모양이 어긋나지 않는다.
// ══════════════════════════════════════════════════════════════════
const PATHS = {
  ...
  'dice': '<rect width="18" height="18" x="3" y="3" rx="3"/><path d="M8.5 8.5h.01"/><path d="M15.5 8.5h.01"/><path d="M8.5 15.5h.01"/><path d="M15.5 15.5h.01"/><path d="M12 12h.01"/>',
  ...
  'glasses-off': '<circle cx="6" cy="15" r="4"/><circle cx="18" cy="15" r="4"/><path d="M14 15a2 2 0 0 0-2-2 2 2 0 0 0-2 2"/><path d="M2.5 13 5 7c.7-1.3 1.4-2 3-2"/><path d="M21.5 13 19 7c-.7-1.3-1.5-2-3-2"/><path d="m3 3 18 18"/>',
  'table': '<path d="M12 3v18"/><rect width="18" height="18" x="3" y="3" rx="2"/><path d="M3 9h18"/><path d="M3 15h18"/>',
};

/**
 * 인라인 SVG 문자열. 크기·색은 부모 글자에 따라간다(currentColor / em).
 * @param name PATHS 키
 * @param size px. 안 주면 1em 이라 텍스트와 같이 커진다.
 */
