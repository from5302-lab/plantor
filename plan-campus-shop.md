...
> **[완료]** 2026-08-12 배포. 1(굽기)·2(배치)·3(배포) 전부 끝.
> 배포 후 사용자 지시로 구조가 한 번 더 바뀌었다: **문을 북→동쪽으로**(밖에서
> 들어온 방향과 이어지게), **현관·칸막이 삭제**(매장 한 덩어리), **계산대·매점쌤을
> 문 앞 북동으로**. buildIndoor 가 현관 없는 건물과 동·서쪽 문을 알게 됐고,
> 현관 없는 건물의 출구 존은 룸 존보다 앞순위여야 한다(안 그러면 못 나간다).
> 아래 §2 의 북쪽 문 기준 좌표는 첫 배포본 기록이다 — 최종 좌표는 map.js 참조.
